...
def _move_file(ctx: ActContext, args: list[str]):
    ...
    if not src_path.exists():
        ctx.fail(bus.render_to_string(L.acts.refactor.error.srcNotFound, path=src_raw))
    ...
    try:
        ...
        ctx.fail(bus.render_to_string(L.acts.refactor.error.movePermission, src=src_raw, dest=dest_raw))
    except Exception as e:
        ctx.fail(bus.render_to_string(L.acts.refactor.error.moveUnknown, error=e))
...
def _delete_file(ctx: ActContext, args: list[str]):
    ...
    if not target_path.exists():
        bus.warning(L.acts.refactor.warning.deleteSkipped, path=raw_path)
        return
    ...
    try:
        ...
        ctx.fail(bus.render_to_string(L.acts.refactor.error.deletePermission, path=raw_path))
    except Exception as e:
        ctx.fail(bus.render_to_string(L.acts.refactor.error.deleteUnknown, error=e))

    bus.success(L.acts.refactor.success.deleted, path=raw_path)
