...
本计划将分两步执行：
1.  **固化成果**: 将上一阶段成功的数据模型升级提交到 Git 历史中。
2.  **实现导航**: 在 `main.py` 中添加 `undo`, `redo`, `prev`, `next` 四个新的 CLI 命令，让用户可以在历史图谱中轻松穿梭。

---

### 步骤 1: 创建提交

我们将把 `core/models.py`, `core/history.py` 的修改以及新增的 `tests/test_graph_loader.py` 文件暂存并提交。
...
~~~~~text
feat(core): 为图导航增强数据模型

- 在 AxonNode 中添加 parent, children 和 siblings 属性，
  使其成为一个真正的图节点。
- 重构 history loader，在加载时构建节点间的连接关系。
- 添加单元测试以验证图加载逻辑的正确性。

这是实现 undo/redo 和 TUI 导航功能的基础。
~~~~~

---

### 步骤 2: 为 `main.py` 添加导航命令

我们将通过一次 `replace` 操作，为 `main.py` 添加新的命令和辅助函数。这些辅助函数将帮助我们保持代码的整洁和复用。
...
脚本执行完毕。我们已经成功地：
1.  保存了数据模型升级的成果。
2.  在 `main.py` 中实现了四个新的导航命令：`undo`, `redo`, `prev`, `next`。

现在，你可以尝试在你的项目目录下运行这些新命令了！例如：
- `python main.py undo`
- `python main.py redo -n 2`
- `python main.py next`

下一步，我们将基于这些坚实的 CLI 命令，开始构建 TUI 界面。