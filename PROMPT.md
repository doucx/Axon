...
#### 参数简写 (Inline Arguments)
为了节省 Token 并提高可读性，你可以将指令的简短参数（如文件名、搜索关键词）直接写在 `act` 块的声明行中。

*   **规则**：对于大多数指令，行内参数会被解析为前缀。但为了安全起见，**不要混合使用**行内参数和块参数。
    *   **推荐**：
        *   要么将所有简短参数写在 `act` 行内（以空格分隔）。
        *   要么 `act` 行留空，将所有复杂内容写在后续的 Block 中。
*   **示例**：
    ~~~~~act
    replace README.md
    ~~~~~
    ...
    (新内容)
    ~~~~~
    *上例中，`README.md` 自动成为参数 1，后续两个块分别成为参数 2 和 3。*
...
*   参数 1: `path`
*   参数 2: `content`
*   **注意**：Axon **不会**自动添加换行符。如果你希望内容另起一行，请在 `content` 块的开头手动包含一个空行。
